...
class MovableCircle:
    ...
    def shrink(self, event):
        if self.is_dragging:
            self.r = np.sqrt((event.x - self.x)**2 + (event.y - self.y)**2)
            if self.r > BASE_FACTOR * ROBOT_RADIUS:
                self.canvas.coords(self.influence_circle, self.x - self.r, self.y - self.r, self.x + self.r, self.y + self.r)
...
def update():
    ...
    for i in range(1, 13, 2):
        ...
        dv = (dv[0]/np.sqrt(dv[0]**2 + dv[1]**2), dv[1]/np.sqrt(dv[0]**2 + dv[1]**2))
        speed = (items[i].r - BASE_FACTOR * ROBOT_RADIUS)/SPEED_FACTOR
        team_speed.append((dv[0] * speed, dv[1] * speed))
        ...
        opp_speed.append((dv[0] * speed, dv[1] * speed))
    goal = (items[0].x, items[0].y)
    path = test_pathfinder(team_position, opp_position, team_speed, opp_speed, goal)
...
def test_pathfinder(team_position, opp_position, team_speed, opp_speed, goal):
    ...
    mock_world.get_team_speed.return_value = [tuple(x * 10 for x in pos) for pos in team_speed]
    mock_world.get_opp_speed.return_value = [tuple(x * 10 for x in pos) for pos in opp_speed]
